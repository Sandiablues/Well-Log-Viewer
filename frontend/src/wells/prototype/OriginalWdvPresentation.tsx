...
import {
  lithologyIntervals21_31,
  lithologySource,
} from './lithologyTrackData';
import type {
  ManagedCurveSamplesByCurveId,
...
function resolveCurveAssignmentCatalogItem(
  ...
  const uidMatch = catalog.find((curve) => (
    Boolean(curve.curveUid) && identityCandidates.includes(String(curve.curveUid))
  ));
  ...
  if (mnemonicCandidates.length === 0) return null;
...
type CurveInventoryWellContext = {
...
function CurveInventory({
  ...
  curveRunMetadata: Map<string, { runInterval?: string | null; runNumber?: string | null }>;
  ...
  const renderCurveRow = (curve: CurveCatalogItem, options?: { duplicateInstance?: boolean }) => {
    ...
    return (
      ...
      >
        ...
        {activeInventoryTab === 'selected' ? (
          <span className="wlv-selected-curve-metadata">
            ...
            {(curveRunMetadata.get(curveIdentityKey)?.runInterval || curveRunMetadata.get(curve.curveId)?.runInterval) && (
              <span className="wlv-selected-curve-interval">
                {curveRunMetadata.get(curveIdentityKey)?.runInterval || curveRunMetadata.get(curve.curveId)?.runInterval}
              </span>
            )}
          </span>
  ...
  return (
    <aside className="wlv-curve-inventory">
      ...
      {activeInventoryTab === 'all' && (
        <div className="wlv-loaded-well-selector">
          <label htmlFor="wlv-loaded-well-select">Well</label>
          <select
            id="wlv-loaded-well-select"
            value={activeWell?.managedWellId ?? ''}
            disabled={loadedWells.length === 0}
            onChange={(event) => onActiveWellChange(event.target.value)}
          >
...
export interface OriginalWdvPresentationProps {
  status: 'idle' | 'loading' | 'loading_samples' | 'ready' | 'mutating' | 'empty' | 'error';
  issues: readonly { message: string }[];
  error: string | null;
  hasLoadedViewerWell: boolean;
  toolbarProps: ComponentProps<typeof Toolbar>;
  curveInventoryProps: ComponentProps<typeof CurveInventory>;
  trackCanvasProps: ComponentProps<typeof TrackCanvas>;
  rightPanelProps: Omit<ComponentProps<typeof RightPanel>, 'wellHeader'>;
  wellHeader: WellHeader;
  trackBackdropMode: TrackBackdropMode;
  renderPropertiesPanel?: (
    props: ComponentProps<typeof RightPanel>,
  ) => ReactNode;
}

export function OriginalWdvPresentation({
  status,
  issues,
  error,
  hasLoadedViewerWell,
  ...
  const [curveInventoryWidthPx, setCurveInventoryWidthPx] = useState(320);
  const [curveInventoryResizeState, setCurveInventoryResizeState] = useState<{
    startX: number;
    startWidth: number;
  } | null>(null);

  useEffect(() => {
    if (!curveInventoryResizeState) return;

    const onMove = (event: MouseEvent) => {
      const next = curveInventoryResizeState.startWidth
        + event.clientX
        - curveInventoryResizeState.startX;
      setCurveInventoryWidthPx(Math.max(220, Math.min(560, next)));
    };
    const onUp = () => setCurveInventoryResizeState(null);
  ...
  const tracks = trackCanvasProps.tracks;

  return (
    <div className="wlv-prototype-root">
      {!hasLoadedViewerWell && (
        <section
          className="wlv-empty-viewer-top-banner"
          aria-label="Well Data Viewer empty state"
        >
          <strong>No data loaded in the Well Data Viewer</strong>
          <span>
            The WDV is ready. Load a managed well or selected products from
            the MDP using Bulk Action → Load selected to Data Viewer.
          </span>
        </section>
      )}
      ...
      <div
        className={`wlv-prototype-workspace wlv-track-backdrop-${trackBackdropMode} ${
          curveInventoryResizeState ? 'curve-inventory-resize-active' : ''
        } ${curveInventoryCollapsed ? 'curve-inventory-collapsed' : ''}`}
        style={{
          gridTemplateColumns:
            `${curveInventoryCollapsed ? 38 : curveInventoryWidthPx}px `
            + 'minmax(0, 1fr) 330px',
        }}
        data-wdv-status={status}
      >
        <div
          className={`wlv-curve-inventory-shell ${
            curveInventoryCollapsed ? 'collapsed' : ''
          }`}
          style={{
            width: curveInventoryCollapsed ? 38 : curveInventoryWidthPx,
          }}
        >
          <button
            type="button"
            className="wlv-curve-inventory-collapse-toggle"
            onClick={() => setCurveInventoryCollapsed((value) => !value)}
            aria-expanded={!curveInventoryCollapsed}
            aria-label={
              curveInventoryCollapsed
                ? 'Expand Curve Inventory'
                : 'Collapse Curve Inventory'
            }
          >
            {curveInventoryCollapsed ? '›' : '‹'}
          </button>
          ...
          {curveInventoryCollapsed ? (
            <div
              className="wlv-curve-inventory-collapsed-label"
              aria-hidden="true"
            >
              Curves
            </div>
            ...
            <>
              {error && <div className="wlv-workspace-error">{error}</div>}
              {issues.map((issue, index) => (
                <div
                  className="wlv-workspace-error"
                  key={`${index}-${issue.message}`}
                >
                  {issue.message}
                </div>
              ))}
              <CurveInventory {...curveInventoryProps} />
            </>
          )}
          ...
          {!curveInventoryCollapsed && (
            <button
              ...
              onMouseDown={(event) => {
                event.preventDefault();
                setCurveInventoryResizeState({
                  startX: event.clientX,
                  startWidth: curveInventoryWidthPx,
                });
              }}
              onDoubleClick={() => setCurveInventoryWidthPx(320)}
            />
          )}
        </div>
        ...
        {tracks.length === 0 ? (
          hasLoadedViewerWell ? (
            <section
              className="wlv-loaded-curves-ready-state"
              aria-label="Loaded curves ready"
            >
              <div className="wlv-loaded-curves-ready-card">
                <h2>Loaded curves are ready</h2>
                <p>
                  Select loaded curves and use Add Track, or drag curves into
                  a manually created curve track.
                </p>
                <p className="wlv-empty-viewer-note">
                  MDP Load does not automatically populate well-log tracks.
                </p>
              </div>
            </section>
          ) : (
            <section
              className="wlv-track-canvas wlv-track-canvas-empty-active"
              aria-label="Blank Well Data Viewer track canvas"
            >
              <div className="wlv-track-strip" aria-hidden="true" />
            </section>
          )
        ) : (
          <TrackCanvas {...trackCanvasProps} />
        )}

        {tracks.length === 0 ? (
          <aside
            className="wlv-right-panel wlv-ready-properties-panel"
            aria-label="Track properties unavailable"
          >
            <div className="wlv-panel-heading">
