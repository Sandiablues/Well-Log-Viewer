...
def get_session(
    ...
    try:
        return service.get_session(managed_well_uid)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc


@router.put("/{managed_well_uid}", response_model=WdvCanonicalSession)
def put_session(
    managed_well_uid: str,
    ...
    try:
        return service.put_session(session)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc


@router.post("/{managed_well_uid}/clear", response_model=WdvCanonicalSession)
def clear_session(
    managed_well_uid: str,
