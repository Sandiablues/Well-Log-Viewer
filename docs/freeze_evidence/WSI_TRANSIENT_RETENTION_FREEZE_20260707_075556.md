# WSI Transient-Retention Freeze Evidence

- Validation timestamp: 20260707_075556
- Branch: feature/wlv-native-las-wdv-20260630
- Git HEAD: 93be4f6b5e6109cdd141fa70c1e14650c142f147
- Acceptance result: PASS
- Backend focused acceptance: 49 passed, 1 skipped
- Frontend Vite production build: PASS

## Frozen WSI behaviour

- External LAS and DLIS sources are read-only references.
- WLV does not move, rename, overwrite, or delete external source files.
- Browser uploads are WLV-owned temporary materializations.
- WSI, WMD, WDV, WBV, export, and saved-workspace references govern retention.
- Unreferenced WLV-owned temporary and derived data can be cleared.
- LAS derived cache can be rebuilt from the external source.
- DLIS is reparsed from the external source and has no persistent sample store.
- WSI-to-WMD authority is transient availability.
- QAQC and metadata overlays can be exported as JSON sidecars.
- Saved workspaces retain references and overlays, not parsed sample arrays.
- Missing, inaccessible, and changed sources are reconciled without source mutation.
- Legacy registration fields remain compatibility aliases only.
...
This freeze validates the dedicated transient-retention acceptance suite. It does not assert that every unrelated legacy WSI test in the repository is current.
