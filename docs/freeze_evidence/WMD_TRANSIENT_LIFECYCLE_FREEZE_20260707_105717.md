# WMD Transient-Lifecycle Freeze Evidence

- Validation timestamp: 20260707_105717
- Branch: feature/wlv-native-las-wdv-20260630
- Git HEAD: 93be4f6b5e6109cdd141fa70c1e14650c142f147
- Acceptance result: PASS
- Focused backend acceptance: 37 passed
- Focused frontend recovery acceptance: 3 passed
- Frontend Vite production build: PASS

## Frozen WMD behaviour

- WMD is an active transient working-data layer rather than a permanent source repository.
- Backend-owned lifecycle fields govern working state, retention state, cleanup eligibility, and retention reason.
- UUIDv7 references for WMD, WDV, WBV, export, and saved workspaces govern retention.
- Removed wells and products become cleanup-eligible only after all retaining references are released.
- Cleanup clears only eligible WLV-owned transient viewer payloads.
- Cleanup preserves canonical well, product, and curve identities.
- Cleanup preserves product rows, external source references, expected fingerprints, QAQC decisions, metadata overlays, and rebuild provenance.
- External LAS and DLIS files remain read-only and are never moved, renamed, overwritten, or deleted.
- Cleared payloads can be rebuilt from the retained external source after source availability and fingerprint verification.
- Rebuild preserves UUIDv7 well, product, and curve identities.
- Missing, changed, and inaccessible sources leave explicit recovery state and do not restore stale payloads.
- Changed fingerprints are observed but are not silently accepted.
- WDV, WBV, export, and saved-workspace readiness are governed by the backend downstream-recovery contract.
- WDV load and viewer-package access reject blocked or cleared payloads.
- WMD, WDV, and WBV render the backend-owned recovery contract without frontend lifecycle authority.
- Normal downstream operation resumes after the original unchanged source is restored and rebuilt.

## Working tree status at validation
...
This freeze validates the dedicated WMD transient-lifecycle acceptance suite and frontend recovery contract. It does not assert that every unrelated legacy repository test is current.
