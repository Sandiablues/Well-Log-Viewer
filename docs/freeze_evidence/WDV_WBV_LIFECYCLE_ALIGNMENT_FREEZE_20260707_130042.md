# WDV/WBV Lifecycle Alignment Freeze Evidence

- Validation timestamp: 20260707_130042
- Branch: feature/wlv-native-las-wdv-20260630
- Git HEAD: 93be4f6b5e6109cdd141fa70c1e14650c142f147
- Acceptance result: PASS
- Backend focused acceptance: PASS
- Frontend recovery acceptance: PASS
- Frontend Vite production build: PASS

## Frozen lifecycle behaviour

- The backend-owned active WDV workspace well governs the WBV session reference.
- Active-well switching immediately moves the WBV UUIDv7 reference.
- Empty workspaces and viewer-session reset release ephemeral WBV references.
- Bulk WDV load and unload transitions reconcile WBV references.
- Saved-workspace references are acquired, reconciled, and fully released through backend commands.
- Cleared, missing, changed, and inaccessible WMD payloads cannot acquire WBV, export, or saved-workspace resume references.
- Export jobs acquire and release UUIDv7-owned WMD references through backend commands.
- Stale export, saved-workspace, and WBV references can be reconciled from backend-authoritative owner sets.
- WMD and persistent WDV references are excluded from stale-consumer cleanup.
- WBV page or session exit does not erase the persistent WDV workspace.
- Existing read-only NOT_LOADED WBV package behaviour remains available when no active WDV well exists.
- WDV and WBV remain consumers of backend lifecycle and recovery contracts; they do not own competing durable lifecycle state.
- Existing viewer-package, trajectory-selection, multi-well workspace, and bulk-load behaviour remain validated.
...
This freeze validates the dedicated WDV/WBV lifecycle-alignment suite, related viewer-session contracts, frontend recovery presentation, and the frontend production bundle. It does not assert that every unrelated repository test is current.
